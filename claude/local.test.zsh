...
claude() {
  ...
  print "SMALL=$ANTHROPIC_SMALL_FAST_MODEL"
...
print "$out" | grep -q 'MODEL=qwen2.5-coder:14b'     || { print "FAIL: main model"; exit 1 }
print "$out" | grep -q 'SMALL=qwen2.5-coder:3b'      || { print "FAIL: small model"; exit 1 }
print "$out" | grep -q 'HAIKU=qwen2.5-coder:3b'      || { print "FAIL: haiku model"; exit 1 }
