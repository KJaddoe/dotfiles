...
description: Use when building a React Native or Expo mobile app, setting up navigation, integrating native modules, improving scroll performance, handling SafeArea or keyboard input, or configuring Expo SDK projects. Invoke for navigation hierarchies (tabs, stacks, drawers), native module configuration, FlatList rendering optimization with memo and useCallback, or platform-specific iOS/Android code.
...
const styles = StyleSheet.create({
  chip: {
    ...
    ...Platform.select({
      ios: { shadowColor: '#000000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4 },
      android: { elevation: 3 },
    }),
  },
