---
name: vue-expert-js
description: Use when building Vue 3 applications with JavaScript only (no TypeScript), when projects require JSDoc-based type hints, when migrating from Vue 2 Options API to Composition API in JS, or when teams prefer vanilla JavaScript, .mjs modules, or quick prototypes without TypeScript setup. Invoke for vanilla JS composables, Vite configuration, routing and state management, or JSDoc type annotations (@typedef, @param, @returns) for full type coverage without a TS compiler.
...
3. **Annotate** - Add JSDoc type annotations (`@typedef`, `@param`, `@returns`, `@type`) for full type coverage; then run ESLint with the JSDoc plugin (`eslint-plugin-jsdoc`) to verify coverage - fix any missing or malformed annotations before proceeding
...
- Type-annotate every public function's parameters and return value with `@param` and `@returns`: plain JavaScript carries no type information of its own, so the type the annotation adds is never something the declaration already shows, and the gate is always cleared for the type itself. Keep any descriptive prose above those tags to one line, three at the absolute most, and drop the block entirely for a function with no parameters and no return value whose name already says what it does
...
- Skip `@param`/`@returns` type annotations on a public function that takes parameters or returns a value
- Write more than a one-line description above a JSDoc block's tags, or add a block to a parameterless, void function the name already explains
