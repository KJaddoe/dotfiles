...
description: Use when implementing features across frontend and backend, building REST APIs with corresponding UI, connecting frontend components to backend endpoints, creating end-to-end data flows from database to UI, or implementing CRUD operations with UI forms - especially where auth, input validation, output encoding, and parameterized queries must stay consistent across every layer. Distinct from frontend-only, backend-only, or API-only skills in that it addresses Frontend, Backend, and Security together in one pass. Invoke for authenticated API routes with views, microservices, real-time features, monorepo architecture, or technology selection decisions.
