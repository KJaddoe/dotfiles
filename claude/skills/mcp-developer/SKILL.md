...
5. **Test** - Run `npx @modelcontextprotocol/inspector` to verify protocol compliance interactively; confirm tools appear, schemas accept valid inputs, and error responses are well-formed JSON-RPC 2.0. **Feedback loop:** if schema validation fails, inspect the Zod/Pydantic error output, fix the schema definition, and re-run the inspector. If a tool call returns a malformed response, check transport serialisation, fix the handler, and re-test.
