...
description: Use when building Kotlin applications requiring coroutines, multiplatform development, or Android with Compose. Invoke for coroutine concurrency, Flow stream handling, KMP projects, Ktor servers, type-safe DSL design, sealed classes, or suspend functions.
...
- Write KDoc only where it says something the declaration doesn't already show (e.g. a nullability contract beyond what the type system encodes, a coroutine cancellation behavior, a documented `!!` justification) - not on every public class/function whose name, signature, and sealed-class hierarchy already tell the whole story
