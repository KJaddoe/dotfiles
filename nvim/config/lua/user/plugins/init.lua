...
require("lazy").setup({
  ...
  },
  {
    ...
    -- first session initializes.
    "mfussenegger/nvim-dap",
    dependencies = { "rcarriga/nvim-dap-ui", "nvim-neotest/nvim-nio" },
    keys = {
      lazy_key("<leader>xb", function()
        require("dap").toggle_breakpoint()
      end, "Toggle breakpoint"),
      lazy_key("<leader>xB", function()
        require("dap").set_breakpoint(vim.fn.input("Breakpoint condition: "))
      end, "Conditional breakpoint"),
      lazy_key("<leader>xc", function()
        require("dap").continue()
      end, "Continue / start"),
      lazy_key("<leader>xi", function()
        require("dap").step_into()
      end, "Step into"),
      lazy_key("<leader>xo", function()
        require("dap").step_over()
      end, "Step over"),
      lazy_key("<leader>xO", function()
        require("dap").step_out()
      end, "Step out"),
      lazy_key("<leader>xr", function()
        require("dap").repl.toggle()
      end, "Toggle REPL"),
      lazy_key("<leader>xt", function()
        require("dap").terminate()
      end, "Terminate session"),
      lazy_key("<leader>xu", function()
        require("dapui").toggle()
      end, "Toggle debug UI"),
      lazy_key("<leader>xk", function()
        require("dapui").eval()
    ...
    config = function()
      ...
      -- js-debug (vscode-js-debug) DAP server, installed by the neovim ansible
      -- role
      local js_debug_path =
      ...
      ---@return string|nil dir Absolute directory, nil when none is found
      ...
      --- `<name>.runtimeconfig.json`, so we scan the solution's Debug output
      --- for those and keep only assemblies whose name matches their owning
      --- project folder, which drops copied dependencies and tooling such as
      --- roslyn's BuildHost. Test assemblies are skipped, the app the current
      --- buffer belongs to wins, and we prompt only when several candidates
      --- still remain.
      ---@return string|nil program Absolute dll path, nil when none is built
      ...
      --- == "Project"` profile in `Properties/launchSettings.json`, which is
      --- where `ASPNETCORE_ENVIRONMENT` and the application URLs live.
      --- Debugging the built dll bypasses that file, so without this the app
      --- boots as Production (no `appsettings.Development.json` and therefore
      --- no dev connection strings) on Kestrel's default port 5000, which on
      --- macOS also collides with Control Centre's AirPlay receiver.
      ---@param project_dir string Directory holding the `.csproj`
      ---@return table<string, string> env Empty when the profile sets none
      local function launch_profile_env(project_dir)
        local file =
          io.open(project_dir .. "/Properties/launchSettings.json", "r")
        if not file then
          return {}
        end
        ...
        -- vim.json.decode drops JSON object order, so pick by sorted name to
        -- keep the same profile winning on every launch.
      ...
      dap.configurations.cs = {
        {
          ...
          --- Run from the owning project directory so ASP.NET resolves
          --- appsettings and content-root relative paths as it would under
          --- `dotnet run`.
          cwd = function()
            return nearest_root("%.csproj$") or vim.fn.getcwd()
          end,
          --- Launching the dll bypasses launchSettings.json, so read the
          --- profile's environment ourselves. The shell still wins where it
          --- sets a value, so `ASPNETCORE_URLS=http://localhost:5179 nvim`
          --- keeps overriding the port.
          env = function()
            local env =
      ...
      -- Stack-agnostic layout (Angular, .NET, Nest, ...): Scopes over Stacks on
      -- the left, REPL + Console along the bottom. The Breakpoints and Watches
      -- panels are dropped -- breakpoints show as gutter signs, and ad-hoc
      -- values come from <leader>xk (eval) and the REPL -- which reclaims the
      -- space they wasted.
      dapui.setup({
        layouts = {
          {
      ...
      -- live-reload terminates the old target while the root session lives on.
      -- A child carries `session.parent`; the root does not. Tear the UI down
      -- only when the root session ends, so a reload keeps the panels open but
      -- a real quit (dap.terminate) closes them. (nvim-dap can't count
      -- remaining sessions here: `before` listeners fire before the terminating
      -- session is removed, so dap.sessions() still holds it and would never
      -- read as empty.)
      ...
      dap.listeners.before.event_terminated["dapui_config"] =
        close_ui_on_root_exit
      dap.listeners.before.event_exited["dapui_config"] = close_ui_on_root_exit

      -- Signs. nvim-dap highlights the stopped line with `debugPC`, which
      -- renders darker than the background (invisible); re-point it at
      -- `DapStoppedLine`, defined in the kanagawa spec above, and give every
      -- marker a visible colour instead of the default low-contrast
      -- `SignColumn`.
