...
| `configuration.md` | Env vars this repo's own tooling reads, the hook inventory, and the permission mode they assume                                                    | Changing hook behaviour, the permission mode, or adding a tool that reads config |
| `nvim-lsp.md`      | LSP navigation, code actions and diagnostics: keymaps, why an empty result is often correct, and how discoverability is wired                      | Changing or looking up nvim LSP behaviour                                        |
