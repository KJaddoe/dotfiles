...
- **Topic folders** (`git/`, `node/`, `claude/`, …): one per area. Files are picked up by
  ...
  - `bin/*` is added to `$PATH`. A script named `git-<name>` there becomes the `git <name>`
    subcommand for free, so that's the home for git helpers with real logic (`bin/git-gone`),
...
- **`_system/`** is the Ansible side: `main.yml` (role list), `roles/<tool>/`, `requirements.yml`,
...
by name only, so extensionless zsh scripts are not covered, unlike the shellcheck shebang scan.
...
- **tmux status bar**: native tmux config in `tmux/tmux.conf`, with no status-bar plugin (see
  ...
  through `$PATH`, so they render empty rather than erroring if the tmux server's environment lacks
...
`apt_key` and `apt_repository` are **deprecated** (removed in ansible-core 2.25), so don't use
